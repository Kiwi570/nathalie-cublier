  ...
  return (
    <article className="group">
      <Link
        ...
        className="paper-card relative block overflow-hidden rounded-[1.5rem] bg-white p-5"
      >
        <div className="relative aspect-[4/5] overflow-hidden bg-mist">
          <Image
            ...
            className="object-contain transition-transform duration-1000 ease-out-expo group-hover:scale-[1.035]"
          />
        </div>
        <span
      ...
      <div className="flex items-start justify-between gap-4 px-1 pt-5">
        <div>
          ...
          <h3 className="mt-1 font-serif text-2xl transition-colors duration-500 group-hover:text-vermilion">
            {artwork.title}
          </h3>
          <p className="mt-1 text-xs text-ink/48">{artwork.format}</p>
        </div>
        <ArrowUpRight className="mt-1 size-5 text-ink/40 transition-[color,transform] duration-500 ease-out-expo group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-vermilion" />
      </div>
    </article>
  );
}
